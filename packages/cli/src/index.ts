      ...
      const answers = await inquirer.default.prompt([
        {
          ...
          mask: "*",
          validate: (val: string) => val.startsWith("oi_") ? true : "Key must start with oi_",
        },
      ]);
...
program
  ...
  .action(async (deployId) => {
    ...
    try {
      ...
      if (s.project_type) { console.log(`  Type:     ${s.project_type}`); }
      if (s.target_url) { console.log(`  URL:      ${chalk.blue(s.target_url)}`); }
      if (s.security_score != null) { console.log(`  Security: ${s.security_score}/100`); }
      console.log();
    } catch (err: any) {
