...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand("otoinstall.deploy", () => deployProject()), vscode.commands.registerCommand("otoinstall.deployZip", (uri) => deployZip(uri)), vscode.commands.registerCommand("otoinstall.checkStatus", () => checkStatus()), vscode.commands.registerCommand("otoinstall.viewLogs", () => viewLogs()), vscode.commands.registerCommand("otoinstall.listServers", () => listServers()), vscode.commands.registerCommand("otoinstall.listProjects", () => listProjects()), vscode.commands.registerCommand("otoinstall.setApiKey", () => setApiKey()));
...
function getApi() {
    ...
    const baseUrl = config.get("baseUrl", "https://otoinstall.com");
    if (!apiKey) {
        vscode.window.showWarningMessage("OtoInstall: API key not configured.", "Set API Key").then((choice) => {
    ...
    return new api_1.OtoInstallAPI(apiKey, baseUrl);
}
function log(msg) {
...
async function setApiKey() {
    const key = await vscode.window.showInputBox({
        ...
        ignoreFocusOut: true,
        validateInput: (val) => {
            if (!val.startsWith("oi_")) {
                return "API key must start with oi_live_ or oi_test_";
            }
            if (val.length < 20) {
...
async function deployProject() {
    ...
    // Confirm deploy
    const confirm = await vscode.window.showInformationMessage(`Deploy "${(0, path_1.basename)(targetFolder.uri.fsPath)}" to ${picked.label}?`, { modal: true }, "🚀 Deploy");
    if (confirm !== "🚀 Deploy") {
        return;
    }
    // Start deployment
    await vscode.window.withProgress({
        ...
        try {
            ...
            const zipPath = await (0, zip_1.zipDirectory)(targetFolder.uri.fsPath);
            ...
            // Result
            if (status === "completed") {
                const action = await vscode.window.showInformationMessage(`🎉 Deployed successfully!`, "View Dashboard", "View Logs");
...
async function deployZip(uri) {
    ...
    const pickItems = servers.map((s) => ({ label: s.label, description: `${s.protocol}://${s.host}`, id: s.id }));
